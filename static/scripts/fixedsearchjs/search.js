// static/scripts/fixedsearch/search.js - Slightly modified version of fixedsearch
/*--------------------------------------------------------------
...
fixedsearch = function(){
	...
	function search_toggle_focus(e) {
		...
		if (!search__focus) {
			...
			search_submit.value = '⌕ Search (Ctrl + K)';
	...
	on first call of search box (Ctrl + K)
	--------------------------------------------------------------*/
	function search_init() {
		if (first_run) {
			load_script('https://cdn.jsdelivr.net/npm/fuse.js@7.0.0').then(() => {
	...
	/*--------------------------------------------------------------
	Using the index we loaded on Ctrl + K, run
	...
	function search_exec(term) {
		...
		if (results.length === 0) { // no results based on what was typed into the input box
			...
			for (let item in results.slice(0,15)) { // only show first 5 results
...
`<li><a href="${results[item].item.permalink}" tabindex="0">
	...
	<span class="author">${results[item].item.author}</span>
